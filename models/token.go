...
func (t *Token) Redeem(pendingPolicies policyStore, policies policyStore, tokens tokenStore) (ret string, userErr error, dbErr error) {
	domain, err := tokens.UseToken(t.Token)
	if err != nil {
		return domain, err, nil
	}
	domainData, err := pendingPolicies.RemovePolicy(domain)
	if err != nil {
		return domain, nil, err
	}
	err = policies.PutOrUpdatePolicy(&domainData)
	return domain, nil, err
}
